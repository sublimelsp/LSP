...
class LspApplyWorkspaceEditCommand(LspWindowCommand):
    ...
    def run(self, session_name: str, edit: WorkspaceEdit, preserve_tabs: bool = False) -> None:
        session = self.session_by_name(session_name)
        if not session:
            debug('Could not find session', session_name, 'required to apply WorkspaceEdit')
            return
        sublime.set_timeout_async(lambda: session.apply_workspace_edit_async(edit, preserve_tabs))
